...
def get_refs_for_root_item(
    item: ast.AST,
    other_root_refs: dict[str, ast.AST],
    checked: set[str] = set(),
    other_root_checked: set[str] = set(),
) -> list[str]:
...
class ExtractedRawInfo:
    def __init__(
        ...
        filename: str,
        imports: list[str],
        file_depencencies: dict[str, set[str]],
...
def extract_raw_info_from_file(
    filename: str, names: list[str], files_checked_for_refs: set[str] = set()
    ...
    imported_refs_from_local_files = set(
        filter(
            lambda a: a in relevant_refs
            and module_to_file_path(filename, parsed_imports[a][0]) is not None,
            imported_refs,
        )
    )

    # If we cannot find it locally then it is likely an external package
    imported_refs_installed = set(
        filter(
            lambda a: a in relevant_refs
            and module_to_file_path(filename, parsed_imports[a][0]) is None,
            imported_refs,
        )
    )
...
def extract_from_file(filename: str, names: list[str]):
    ...
    def comp_a_b(a: str, b: str):
        ...
        return len(a_data.file_dependencies.keys()) - len(
            b_data.file_dependencies.keys()
        )
...
if __name__ == "__main__":
    ...
    with open(args.output, "w", encoding="utf8") as out_file:
        file_imports, file_content = extract_from_file(
